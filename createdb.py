...
def rent_cup(
    ...
    try:
        ...
        cursor.execute(f"UPDATE vendors_db SET cup_stock = stock.value - 1 FROM (SELECT cup_stock AS value FROM vendors_db WHERE vendor_id = '{vendor}') AS stock WHERE vendors_db.vendor_id = '{vendor}';")
        connection.commit()
